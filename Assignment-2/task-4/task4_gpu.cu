...
/* ── GPU driver ──────────────────────────────────────────────────────────── */
void multiplyGPU(const float *A_h, const float *B_h, float *C_h,
    ...
    /* record start — includes H2D transfer */
    ...
    double elapsed = gpuMs * 1000.0;   /* ms to us */

    cout << "Size: " << rowsA << "x" << colsA << "x" << colsB
         << "  GPU Time (H2D+kernel+D2H): " << elapsed << " us\n";

    /* append to gpu_timings.csv */
    ofstream csv("gpu_timings.csv", ios::app);
    if (csv.is_open()) {
...
int main(int argc, char *argv[]) {
    ...
    multiplyGPU(A, B, C, rowsA, colsA, colsB);
